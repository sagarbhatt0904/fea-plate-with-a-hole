%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                        %
% Elastic 4-node Quadralateral Elements                            %
%                                                                        %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for no=1:5
    ...
    for i=1:2:size(el_list(:,1))
        for j=1:size(NBC(:,1))
            ...
            k=0;
            if (NBC(j,1)==el_list(i,1))
                NBC(j,2:3)=[el_list(i,2) el_list(i+1,2)];
            end
        end
    end
    NBC(:,4)=1;
    ...
    i_hol=1;
    for i=1:E
        if (hole(i_hol)==Elems(i,4)||hole(i_hol)==Elems(i,5)||hole(i_hol)==Elems(i,6)||hole(i_hol)==Elems(i,7))
    ...
    NG = 4;
    [XG,WG] = Q4_El_Gauss_Points(NG);
    
    % Loop over Q4 elements
    for e = 1:E
        ...
        [Ke] = Q4_El_Stiff(ipstrn,xy,h,Y,nu,udof,NE,NG,XG,WG);
    ...
    NES = 2;
    % Set Gauss point locations and weights for traction integration
    NGS = 2;
    [XGS,WGS] = Q4_El_Gauss_Points_Surf(NGS);
    ...
    for q = 1:Q
        ...
        for i=1:NGS
            ...
            [NshapeS] = Q4_El_Shape_Surf(NES,xi);
            [DNshapeS] = Q4_El_DShape_Surf(NES,xi);
            
            xyS(1,1) = Nodes(in1,2);
            xyS(1,2) = Nodes(in1,3);
            xyS(2,1) = Nodes(in2,2);
            xyS(2,2) = Nodes(in2,3);
            [detJS] = Q4_El_Jacobian_Surf(NES,xi,xyS,DNshapeS);
    ...
    for e = 1:E
        ...
        u = Disp(e,:)';
        [eps,sig] = Q4_El_Str(ipstrn,xy,u,h,Y,nu,udof,NE,NG,XG);
        %eps
        %sig
    ...
    PE(no,2)=3/N;
    if(no==1)
        str=sprintf('Original plate vs deformed plate using 4 Noded Quad elements for %d elements',E);
        figure;
        Plot_deformation;
        title(str);
